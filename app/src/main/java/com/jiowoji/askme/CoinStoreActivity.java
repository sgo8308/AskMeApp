    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imageButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                    if(gotIntent.getStringExtra("fromWhere").equals("PeoplePostActivity")){
                        Intent intent = new Intent(getApplicationContext(),PeoplePostAcitivity.class);
                        intent.putExtra("id",gotIntent.getStringExtra("id"));
                        intent.putExtra("job",gotIntent.getStringExtra("job"));
                        intent.putExtra("nickName",gotIntent.getStringExtra("nickName"));
                        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        startActivity(intent);
                    }else if(gotIntent.getStringExtra("fromWhere").equals("ExperiencePostActivity")){
                        Intent intent = new Intent(getApplicationContext(),ExperiencePostActivity.class);
                        intent.putExtra("postNumber",gotIntent.getIntExtra("postNumber",0));
                        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        startActivity(intent);
                    }
            }
...
//                        PaymentHistoryData paymentHistoryData = gson.fromJson(message, PaymentHistoryData.class);
//                        //결제내역 아이디에 저장
...
//                        ArrayList<PaymentHistoryData> paymentData = new ArrayList<>();
//                        if(paymentDatas.get(nowLogInId) !=null){
//                            paymentData = paymentDatas.get(nowLogInId);
//                        }
//                        paymentData.add(paymentHistoryData);
//                        paymentDatas.put(nowLogInId,paymentData);
//                        SharedPreferencesHandler.saveData(getApplicationContext(),SharedPreferencesFileNameData.PaymentHistoryData,paymentDatas);
//                        Log.i("알림", paymentHistoryData.getReceipt_id());
